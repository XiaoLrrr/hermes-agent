...
def mount_spa(application: FastAPI):
    ...
    def _current_session_token() -> str:
        # SSH/Desktop applies its per-spawn token in start_server(), after this
        # route is mounted during module import.  Read the module attribute at
        # request time instead of closing over the earlier random token.
        ...
        async def no_frontend(full_path: str):
            ...
            if full_path == "" and not gated:
                return HTMLResponse(
                    ...
                    f"window.__HERMES_SESSION_TOKEN__={json.dumps(_current_session_token())};"
    ...
    def _serve_index(prefix: str = ""):
        ...
        token_js = "" if gated else f'window.__HERMES_SESSION_TOKEN__="{_current_session_token()}";'
        bootstrap_script = (
            f"<script>{token_js}"
